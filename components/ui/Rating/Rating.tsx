...
const Rating: React.FC<Props> = ({ rating, ratingAlwaysShown, disabled, label, mod, onChange }) => {
  const ratingToShow = ratingConverter(rating)

  return (
    <div className={`rating ${mod ? "rating--" + mod : ""}`}>
      {!rating && !onChange ? (
        <p className={`rating__newbie`}>
        ...
        <>
          {label && (rating || ratingAlwaysShown) && <span className={"rating__label"}>{ratingToShow}</span>}
          <Rate
            defaultValue={0}
