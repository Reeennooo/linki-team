export function getMD5hash(str) {
  ...
  function M(d) {
    ...
    for (let r = 0; r < d.length; r++) (_ = d.charCodeAt(r)), (f += m.charAt((_ >>> 4) & 15) + m.charAt(15 & _))
    return f
  }
  function X(d) {
  ...
  const dD = unescape(encodeURIComponent(str))
  return M(V(Y(X(dD), 8 * dD.length))).toLowerCase()
}
